...
#linearly interpolate kernels at x,y not used
def interp_kernel_at_xy(x,y,kernels):

	grid_x,grid_y = kernels.shape[:2]

	x_pos = x*(grid_x-1)
	y_pos = y*(grid_y-1)
	x_low = int(np.floor(x_pos))
	x_high = int(np.ceil(x_pos))

	y_low = int(np.floor(y_pos))
	y_high = int(np.ceil(y_pos))

	wx = x_pos - x_low
	wy = y_pos - y_low

	kernel_t = kernels[x_low,y_low]*(1-wx) + kernels[x_high,y_low]*wx
	kernel_b = kernels[x_low,y_high]*(1-wx) + kernels[x_high,y_high]*wx

	kernel = kernel_t*(1-wy) + kernel_b*wy
	return kernel

def uniformConv2d(image,kernel,padding=None):
	image = image.astype(np.float32)
	if padding is not None:
		image = np.pad(image,padding)

	r = convolve2d(image[:,:,0],kernel[:,:,0],'valid')
	g = convolve2d(image[:,:,1],kernel[:,:,1],'valid')
	b = convolve2d(image[:,:,2],kernel[:,:,2],'valid')
	return np.stack((r,g,b),axis=-1)

def FFTblur2d(image,kernel):
	w,h = image.shape[:2]
	image = image.astype(np.float32)
	image = image.transpose(2,0,1)[np.newaxis,...]
	image = torch.as_tensor(image)
	FX = torch.rfft(image,2,onesided=False)
	kernel = kernel.transpose(2,0,1)[np.newaxis,...]
	kernel = torch.as_tensor(kernel)
	FK = p2o(kernel,(w,h))
	FKFX = cmul(FK,FX)
	blur = torch.irfft(FKFX,2,onesided=False)
...
'''
def pixelConv2d(patch,kernels,pad_mode='constant'):
	patch = patch.astype(np.float64)
	kernel_sz = kernels.shape[2]
	output = np.zeros_like(patch)
	W,H = patch.shape[:2]

	pad_dim = ((kernel_sz//2,kernel_sz//2),(kernel_sz//2,kernel_sz//2),(0,0))
	patch = np.pad(patch,pad_dim,pad_mode)
	#
	for w_ in range(W):
		for h_ in range(H):
			#get kernel at xy
			k = interp_kernel_at_xy(1.0*w_/(W-1),1.0*h_/(H-1),kernels)
			p = patch[w_:w_+kernel_sz,h_:h_+kernel_sz]
			output[w_,h_] = np.sum(k*p,axis=(0,1))
	return output
