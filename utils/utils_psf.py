...
def PSFprojection(walk, params):
    ...
    for iImg in range(walk.shape[2]):
        ...
        for i in range(walk.shape[0]):
            ...
            x = m[0, :] / m[2, :]
            y = m[1, :] / m[2, :]
            L = (np.round(x) < 1) | (np.round(x) > crop) | (
                np.round(y) < 1) | (np.round(y) > crop)
            I = np.logical_not(L)
            kernel[np.round(x[I]).astype(int)-1, np.round(y[I]).astype(int) -
                   1] = kernel[np.round(x[I]).astype(int)-1, np.round(y[I]).astype(int)-1] + 1
...
def Schuler_PSF():
    ...
    psf_img = cv2.imread(
        '//home/xiu/workspace/dwgan/MPI_data/bench/blind_psf.png')
...
def Schuler_PSF_facade():
    ...
    for h_ in range(grid_h):
        for w_ in range(grid_w):
            PSF[h_, w_] = psf_img[1+(k_sz+1)*h_:(k_sz+1)
                                  * (h_+1), 1+(k_sz+1)*w_:(k_sz+1)*(w_+1)]
...
def PSF_toy():
    ...
    psf_img_r = cv2.imread(
        '/home/xiu/workspace/dwgan/MPI_data/toy/kernels_channels_1_nm_regularized.jpg')
    psf_img_g = cv2.imread(
        '/home/xiu/workspace/dwgan/MPI_data/toy/kernels_channels_3_nm_regularized.jpg')
    psf_img_b = cv2.imread(
        '/home/xiu/workspace/dwgan/MPI_data/toy/kernels_channels_4_nm_regularized.jpg')
    ...
    for h_ in range(grid_h):
        for w_ in range(grid_w):
            PSF[h_, w_, :, :, 0] = psf_img_r[(
                k_sz+2)*h_+1:(k_sz+2)*(h_+1)-1, 1+(k_sz+2)*w_:(k_sz+2)*(w_+1)-1, 0]
            PSF[h_, w_, :, :, 1] = psf_img_g[(
                k_sz+2)*h_+1:(k_sz+2)*(h_+1)-1, 1+(k_sz+2)*w_:(k_sz+2)*(w_+1)-1, 1]
            PSF[h_, w_, :, :, 2] = psf_img_b[(
                k_sz+2)*h_+1:(k_sz+2)*(h_+1)-1, 1+(k_sz+2)*w_:(k_sz+2)*(w_+1)-1, 2]
...
def PSF_plano_resize():
    ...
    k_sz = 49
    psf_img_r = cv2.imread(
        '/home/xiu/workspace/dwgan/Supplement_SimpleLensImaging_Heide2013/supplemental_material/images/single_lens/plano_convex_lens/psfs/kernels_channels_1_nm_regularized.jpg')
    psf_img_g = cv2.imread(
        '/home/xiu/workspace/dwgan/Supplement_SimpleLensImaging_Heide2013/supplemental_material/images/single_lens/plano_convex_lens/psfs/kernels_channels_3_nm_regularized.jpg')
    psf_img_b = cv2.imread(
        '/home/xiu/workspace/dwgan/Supplement_SimpleLensImaging_Heide2013/supplemental_material/images/single_lens/plano_convex_lens/psfs/kernels_channels_4_nm_regularized.jpg')
...
def PSF_Motion(img_id=0):
    ...
    img_f = '/home/xiu/databag/deblur/ICCV2021/motion_blur/KernelsAsMat/Non_Stationary_kernel_trajectory_{}.mat'.format(
        img_id)
    ...
    for h_ in range(grid_h):
        for w_ in range(grid_w):
            PSF[h_, w_, :, :, 0] = psf_img[50 *
                                           (h_+1)-24:50*(h_+1)+25, 50*(w_+1)-24:50*(w_+1)+25]
            PSF[h_, w_, :, :, 1] = psf_img[50 *
                                           (h_+1)-24:50*(h_+1)+25, 50*(w_+1)-24:50*(w_+1)+25]
            PSF[h_, w_, :, :, 2] = psf_img[50 *
                                           (h_+1)-24:50*(h_+1)+25, 50*(w_+1)-24:50*(w_+1)+25]
