...
def main():
	...
	#ab_numpy[:,0,:,:] = 0.01
	ab_numpy = np.loadtxt('ab_facade_gan.txt').astype(np.float32).reshape(8,12,11,3)
	...
	# 	cv2.imwrite('/home/xiu/workspace/dwgan/MPI_data/facade/z5-{}.png'.format(i),zk)
	# 	cv2.imwrite('/home/xiu/workspace/dwgan/MPI_data/facade/x5-{}.png'.format(i),xk)
	# 	print(i)
	img_E = img_E_denoise[-1].astype(np.uint8)
	cv2.imshow('EE',img_E)
	cv2.waitKey(-1)
	#img_E = img_E.astype(np.uint8)
	#cv2.imwrite('/home/xiu/workspace/dwgan/MPI_data/facade/E.png',img_E)
