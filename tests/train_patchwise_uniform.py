...
def main():
	...
	for epoch in range(n_epoch):
		...
		for iteration in range(len(train_imgs)//n_batch):
			...
			for img in imgs:
				...
				for _ in range(n_patch*n_patch):
					px = np.random.randint(0,PSF_grid.shape[0])
					py = np.random.randint(0,PSF_grid.shape[1])
			...
			loss = 0
			k = util.single2tensor4(mean_PSF)
			k = k.to(device)
		...
		torch.save(model.state_dict(),os.path.join(root,'usrnet_patchwise_uniform_epoch{}.pth'.format(epoch)))
