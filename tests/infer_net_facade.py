...
def main():
	...
	model_code = 'iter1500'
	loaded_state = torch.load('/home/xiu/databag/deblur/models/facade/uabcnet_{}.pth'.format(model_code))
	...
	#for img_id in range(1,12):
		...
		img_L = cv2.imread('/home/xiu/databag/deblur/ICCV2021/MPI_data/facade/blurry.jpg')
		...
		#ab_numpy[:,0,:,:] = 0.01
		ab_numpy = np.loadtxt('/home/xiu/databag/deblur/models/facade/ab_{}.txt'.format(model_code)).astype(np.float32).reshape(gx,gy,stage*2,3)
		...
		for py_start in range(0,gy-gx+1,1):
			...
			if py_start==0 or py_start==gy-gx:
				full_img[px_start*W//gx:(px_start+num_patch[0])*W//gx,py_start*H//gy:(py_start+num_patch[1])*H//gy,:] = patch_E
			else:
				full_img[px_start*W//gx:(px_start+num_patch[0])*W//gx,py_start*H//gy:(py_start+num_patch[1])*H//gy,:] = patch_E
