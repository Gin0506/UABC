...
def main():
	...
	PSF_grid = np.load('./data/Schuler_PSF_bridge.npz')['PSF']
	...
	model.load_state_dict(torch.load('usrnet_bridge_gan.pth'), strict=True)
	model.eval()
	#model.train()
	...
	#ab_numpy[:,0,:,:] = 0.01
	ab_numpy = np.loadtxt('ab_bridge_gan.txt').astype(np.float32).reshape(8,10,2*num_stage+1,3)
	...
	#while running:
	for px_start in range(0,8-4+1,1):
		for py_start in range(0,10-4+1,1):
			...
			cd = cd.view(num_patch**2,2*num_stage+1,3)
...
#		cv2.imwrite('/home/xiu/workspace/dwgan/MPI_data/bridge/z5-{}.png'.format(i),zk)
#		cv2.imwrite('/home/xiu/workspace/dwgan/MPI_data/bridge/x5-{}.png'.format(i),xk)
	img_E = img_E_denoise[-1].astype(np.uint8)
	cv2.imshow('Bridge',img_E)
	cv2.waitKey(-1)
